...
function GlockTracker:PrintShadowlandsInventory(playerName, itemsGroup)
    ...
    if (GlockTracker:CanShowSlHerbs()) then
        GlockTracker:PrintSlCloth(self.db.global[playerName].Shadowlands.Herbs, playerGroup);
    end

    if (GlockTracker:CanShowSlFish()) then
        GlockTracker:PrintSlFish(self.db.global[playerName].Shadowlands.Fish, playerGroup);
    end

    if (GlockTracker:CanShowSlMeat()) then
        GlockTracker:PrintSlMeat(self.db.global[playerName].Shadowlands.Meat, playerGroup);
    end

    if (GlockTracker:CanShowSlCloth()) then
        GlockTracker:PrintSlCloth(self.db.global[playerName].Shadowlands.Cloth, playerGroup);
    end

    if (GlockTracker:CanShowSlOre()) then
        GlockTracker:PrintSlOre(self.db.global[playerName].Shadowlands.Ore, playerGroup);
    end

    if (GlockTracker:CanShowSlLeather()) then
        GlockTracker:PrintSlLeather(self.db.global[playerName].Shadowlands.Leather, playerGroup);
    end
